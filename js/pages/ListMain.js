...
export default {
    ...
    <main v-else class="page-list-new page-with-hero ull2">
        <div class="u-phero">
            <div class="u-phero__body">
                <h1>Main List</h1>
                <p>The Main List contains all levels that are considered to be rate-worthy and as a result have a higher chance of being added to the Demonlist if they are verified.</p>
            </div>
        ...
        <div class="level-container-new surface">
            <LevelPanel :level="level" :all-paths="allPaths" current="main"></LevelPanel>
        </div>
    ...
    async mounted() {
        ...
        for (const key in list1) {
            // Verified levels always appear on the Main List, even if isMain is false.
            if (list1[key][0]?.isMain || list1[key][0]?.isVerified) this.list.push(list1[key]);
        }
