#!/usr/bin/env node
// Builds two things from the templates in design/home/templates/:
//
//   preview.html   the review deck — the four templates on one scrollable page,
//                  the desktop pair in a browser frame and the phone pair in a
//                  390×844 phone, each with its notes beside it
//   pages/*.html   each template as a standalone page, no review chrome, the
//                  way the route would render it
//
...
// file. The shared kit and the chrome blocks live once in templates/_blocks.html
// and are stamped into every template that asks for them.
//
...
const ORDER = [
    { name: 'spotlight', letter: 'A', file: 'a-spotlight' },
    { name: 'board', letter: 'B', file: 'b-board' },
    { name: 'deck', letter: 'C', file: 'c-deck' },
    { name: 'segments', letter: 'D', file: 'd-segments' },
];

const esc = (s) => String(s).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');

// ── The shared kit and chrome ────────────────────────────────────────────────
...
const pages = ORDER.map(({ name, letter, file }) => {
    ...
    for (const key of ['title', 'tag', 'surface', 'files', 'idea', 'weight', 'cost', 'watch']) {
        if (!meta[key]) throw new Error(`${file}.html has no "${key}" in its meta block`);
    }
    if (meta.surface !== 'desktop' && meta.surface !== 'mobile') {
        throw new Error(`${file}.html has surface "${meta.surface}" — expected desktop or mobile`);
    }
...
const notes = (p) => `
            <div class="pg__notes">
                <div class="note note--weight">
                    <div class="note__k">What it fixes</div>
                    <p>${p.meta.weight}</p>
                </div>
...
const sections = pages.map((p) => {
    ...
    if (p.meta.surface === 'mobile') {
        return `
    <section class="pg pg--phone" id="${p.name}">${head}
        <div class="pg__cols">
${notes(p)}
            <div class="phone">
...
${p.body}
    ...
    return `
    <section class="pg" id="${p.name}">${head}
${notes(p)}
        <div class="frame">
            <div class="frame__bar"><span></span><span></span><span></span><code>ull.pages.dev/</code></div>
...
for (const p of pages) console.log(`  pages/${p.file}.html  (${kb(path.join(outDir, `${p.file}.html`))} KB)  ${p.meta.surface}`);
